...
router.get('/', async (req, res) => {
  try {
    ...
    if (page || limit || q) {
      ...
      const sql = `SELECT id, seller_id AS sellerId, name AS nom, price AS prix, quantity, unit, city, country, image, description, date_created AS date FROM products ${where} ORDER BY date_created DESC LIMIT ? OFFSET ?`;
