...
router.post('/:id/boutique', upload.single('logo'), async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      boutique.logo = `/uploads/profile-photos/${req.file.filename}`;
...
router.get('/:id/products', async (req, res) => {
  const { id } = req.params;
  try {
    const [rows] = await pool.query('SELECT id, seller_id, name, price, quantity, unit, city, country, image, description, date_created FROM products WHERE seller_id = ?', [id]);
...
router.post('/:id/products', upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      product.image = `/uploads/profile-photos/${req.file.filename}`;
    } else if (req.body.image) {
      product.image = req.body.image;
    }
    const [result] = await pool.query('INSERT INTO products (seller_id, name, price, quantity, unit, city, country, image, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [product.seller_id, product.name, product.price, product.quantity, product.unit, product.city, product.country, product.image, product.description]);
